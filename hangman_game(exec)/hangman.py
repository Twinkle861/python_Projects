# #hangman
# count check no. of options user has max count =6
# exec->dynamic execution of python program
# if write .place while defining it then its value becomes constant cant change it
# active background seen when click button
    ...
    def check(letter,button):
        ...
        if letter in selected_word:
            ...
            if count == 6:
                ...
                answer = messagebox.askyesno('GAME OVER', x)
                if answer == True:
                    run = True
                    score = 0
                    root.destroy()
