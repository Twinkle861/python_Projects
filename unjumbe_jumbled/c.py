# #jumbled
# in btn=>relief so that it is groved
# ipadx=>internal padding on both sides
# pad gives padding on both sides
...
def res():
    ...
    var = e1.get()
    if var != answers[num]:
        messagebox.showinfo("Correct answer",f"Correct answer was {answers[num]}")
    num = random.randrange(0, len(words), 1)
    lbl.config(text = words[num])
    e1.delete(0, END)
...
root.title("Jumble the unjumbed words")
